(async function () {
    ...
    let archiveSelector = document.querySelector('form [id=archiveSelector]'),
        ...
        contactingServer = document.querySelector('.uploadPackage-contactingServer'),
    ...
    submit.addEventListener('click', async function () {
        ...
        const package = encodeURIComponent(packageName.value)

        submit.classList.add('button--disabled');
        contactingServer.classList.add('uploadPackage-contactingServer--show');

        try {
            const response = await fetch(`/v1/packages/${package}?isArchive=${isArchive}&removeFirstDirectoryFromPath=${removeFirstDirectoryFromPath}`, {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                window.location = `/package/${package}`;
            } else {
            ...
            contactingServer.classList.remove('uploadPackage-contactingServer--show');
