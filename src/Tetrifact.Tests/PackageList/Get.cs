...
namespace Tetrifact.Tests.PackageList
{
    public class Get : Base
    {
        ...
        {
            Directory.CreateDirectory(Path.Combine(Settings.ProjectsPath, "some-project", Constants.PackagesFragment, "package_one"));
            Directory.CreateDirectory(Path.Combine(Settings.ProjectsPath, "some-project", Constants.PackagesFragment, "invalidPackage"));

            File.WriteAllText(Path.Combine(Settings.ProjectsPath, "some-project", Constants.PackagesFragment, "package_one", "manifest.json"), JsonConvert.SerializeObject(new Manifest()));
            // write a manifest file that consists of invalid JSON
            File.WriteAllText(Path.Combine(Settings.ProjectsPath, "some-project", Constants.PackagesFragment, "invalidPackage", "manifest.json"), "definitely not some json");
